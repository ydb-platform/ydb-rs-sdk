...
pub mod stream_read_message {
    ...
    pub struct ReadRequest {
        ...
        /// 1. Server may return one ReadResponse with bytes_size = 70 and than another 80 bytes response;
        ///    now client buffer has 50 free bytes, server is free to send up to 50 bytes in responses.
        /// 1. Client processes 100 bytes from buffer, now buffer free space is 150 bytes,
        ///    so client sends ReadRequest with bytes_size = 100;
        /// 1. Server is free to send up to 50 + 100 = 150 bytes. But the next read message is too big,
        ///    and it sends 160 bytes ReadResponse.
        /// 1. Let's assume client somehow processes it, and its 200 bytes buffer is free again.
