...
mod tests {
    ...
    fn successful_part() -> ExecuteQueryResponsePart {
    ...
    fn successful_result_part() -> ExecuteQueryResponsePart {
        let column_type =
        ...
        ExecuteQueryResponsePart {
            ...
            result_set: Some(ydb_grpc::ydb_proto::ResultSet {
                ...
                rows: vec![ydb_grpc::ydb_proto::Value {
                    items: vec![
                        crate::grpc_wrapper::raw_table_service::value::RawValue::Int64(1).into(),
                    ],
                    ..Default::default()
                }],
                ..Default::default()
    ...
    fn failed_part() -> ExecuteQueryResponsePart {
        ExecuteQueryResponsePart {
            status: StatusCode::BadSession as i32,
            issues: Vec::new(),
            result_set_index: 0,
            result_set: None,
            exec_stats: None,
    ...
    #[tokio::test]
    async fn observing_eof_returns_the_client_session() {
        ...
        let opened = OpenedClientQueryStream {
            stream: ExecuteQueryStream::from_test_parts(vec![successful_part()]),
        ...
        assert!(
            stream
                .next_result_set()
                .await
                .expect("observe stream EOF")
                .is_none()
        );
        assert_eq!(pool.stats().in_use, 0);

        let reused = pool.acquire_explicit().await.expect("reacquire session");
        assert_eq!(reused.session_id(), session_id);
        reused.return_to_pool();
    }

    #[tokio::test]
    async fn final_result_set_returns_the_client_session_before_none() {
        let pool = SessionPool::new_explicit_bench(
            SessionPoolSettings::new().with_limit(1).with_warm_up(1),
        );
        let lease = pool.acquire_explicit().await.expect("acquire test session");
        let session_id = lease.session_id().to_string();
        let opened = OpenedClientQueryStream {
            stream: ExecuteQueryStream::from_test_parts(vec![successful_result_part()]),
            session: ClientQuerySession::Pooled(lease),
        };
        let mut stream = QueryStream::from_client(opened);
    ...
    #[tokio::test]
    async fn dropping_before_eof_discards_the_client_session() {
        let pool = SessionPool::new_explicit_bench(
            SessionPoolSettings::new().with_limit(1).with_warm_up(1),
        );
        let lease = pool.acquire_explicit().await.expect("acquire test session");
        ...
        drop(QueryStream::from_client(opened));
    ...
    #[tokio::test]
    async fn stream_error_discards_the_client_session_before_later_eof() {
        let pool = SessionPool::new_explicit_bench(
            SessionPoolSettings::new().with_limit(1).with_warm_up(1),
        );
        let lease = pool.acquire_explicit().await.expect("acquire test session");
        let session_id = lease.session_id().to_string();
        let opened = OpenedClientQueryStream {
            stream: ExecuteQueryStream::from_test_parts(vec![failed_part()]),
            session: ClientQuerySession::Pooled(lease),
        };
        let mut stream = QueryStream::from_client(opened);
        ...
        stream
            .next_result_set()
            .await
            .expect_err("failed response part must fail the stream");
        assert!(
            stream
                .next_result_set()
                .await
                .expect("terminal stream must remain readable")
                .is_none()
        );
    ...
    #[tokio::test]
    async fn close_drains_before_returning_the_client_session() {
        let pool = SessionPool::new_explicit_bench(
            SessionPoolSettings::new().with_limit(1).with_warm_up(1),
        );
        let lease = pool.acquire_explicit().await.expect("acquire test session");
        ...
        QueryStream::from_client(opened)
            .close()
            .await
            .expect("close stream");
