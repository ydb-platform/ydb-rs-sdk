...
use crate::parallel_endpoint_connect::normalize_uri_scheme;
use crate::YdbResult;
...
async fn test_connection_creates_new_connection() -> YdbResult<()> {
    ...
    let uri = Uri::from_static("grpc://127.0.0.1:7135/path");
...
async fn test_connection_connection_reuse() -> YdbResult<()> {
    let pool = ConnectionPool::new();
    let uri = Uri::from_static("grpcs://127.0.0.1:2135/local");
