...
#[derive(Debug)]
pub(crate) enum GetMessagesToSendWithLengthThresholdResult {
    Ok(Vec<MessageData>),
...
impl MessageQueue {
    ...
    pub(crate) fn get_length_of_messages_to_send(&self) -> YdbResult<usize> {
    ...
    pub(crate) fn get_messages_to_send(&mut self) -> YdbResult<Vec<MessageData>> {
        let length = self.get_length_of_messages_to_send()?;

        Ok(MessageQueue::do_get_messages_to_send(
            &self.messages,
    ...
    pub(crate) fn get_messages_to_send_with_length_threshold(
        &mut self,
        length_threshold: usize,
    ) -> GetMessagesToSendWithLengthThresholdResult {
        let length = match self.get_length_of_messages_to_send() {
            Ok(length) => length,
            Err(error) => return GetMessagesToSendWithLengthThresholdResult::Err(error),
        };

        if length < length_threshold {
            return GetMessagesToSendWithLengthThresholdResult::NotEnoughMessages;
        }

        GetMessagesToSendWithLengthThresholdResult::Ok(MessageQueue::do_get_messages_to_send(
            &self.messages,
            &mut self.last_sent_index,
            length,
        ))
...
mod tests {
    ...
    fn get_messages_to_send_with_length_threshold_ok_when_at_threshold() {
        ...
        match &result {
            GetMessagesToSendWithLengthThresholdResult::Ok(msgs) => {
                assert_eq!(msgs.len(), 2);
    ...
    fn get_messages_to_send_with_length_threshold_ok_when_above_threshold() {
        ...
        match &result {
            GetMessagesToSendWithLengthThresholdResult::Ok(msgs) => assert_eq!(msgs.len(), 3),
            other => panic!("expected Ok(...), got {:?}", other),
        }
    }
