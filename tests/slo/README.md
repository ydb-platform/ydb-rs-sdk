...
| `--operation-timeout` | 120000 | Chain advance and supporting YDB operation deadline (ms) |
...
logical transaction, expressed as a percentage. Latency and throughput include
only confirmed commits. A terminal SDK or YDB error returned before the commit
phase is an operational failure; an error from the commit phase is a commit-phase
failure. Both reduce availability. Invalid chain state immediately fails the
workload and is not represented as a percentage metric. Atomicity and worker
progress remain final pass/fail invariants.

Topic path: `{database}/{WORKLOAD_NAME}/{WORKLOAD_REF}-topic`.
Transition table path: `{database}/{WORKLOAD_NAME}/{WORKLOAD_REF}-transitions`.
