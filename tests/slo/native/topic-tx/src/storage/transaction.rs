...
use super::queries::{read_next_transition, upsert_transition};
...
enum ChainAdvanceOutcome {
    Committed,
    AmbiguousCommit(MessageCoordinate),
...
impl PartitionWorker {
    ...
    pub(crate) async fn run(
        ...
        while !cancel.is_cancelled() {
            ...
            match report.outcome {
                ...
                ChainAdvanceOutcome::AmbiguousCommit(coordinate) => {
                    let message =
                        format!("transaction commit outcome is ambiguous at {coordinate}");
                    span.finish(Some("ambiguous_commit"), report.attempts);
    ...
    async fn advance_chain_once(&mut self) -> ChainAdvanceReport {
        ...
        let mut attempts = 0;
        // Set only after the callback completes all transactional work. If
        // `retry_tx` then fails, the implicit commit outcome is ambiguous.
            ...
            .retry_tx(closure!(
                ...
                async |tx: &mut Transaction| {
                    ...
                    *commit_candidate = None;
                    ...
                    upsert_transition(tx, table_path.as_str(), &transition).await?;
                    ...
                    *commit_candidate = Some(transition.coordinate);
                    Ok(())
                }
            ))
            .idempotent(true)
        ...
        let outcome = match result {
            ...
            Err(error) => match commit_candidate {
                Some(coordinate) => ChainAdvanceOutcome::AmbiguousCommit(coordinate),
